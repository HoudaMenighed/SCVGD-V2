...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = attn.softmax(dim=-1) # to obtain weights
        attn = self.attn_drop(attn) # eviter le overfitting

        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # the new representation
        x = self.proj(x) # to return to the original shape representation
        x = self.proj_drop(x)
        return x
...
# from image to patches projected
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) #(224/16 x 224/16)
        self.img_size = img_size
        self.patch_size = patch_size
        self.num_patches = num_patches

        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size) #Instead of
        # using loops to extract patches manually, we use a 2D convolution with:
        #kernel_size = patch_size, stride = patch_size, The image is divided into non-overlapping patches.
        # Each patch is mapped to a vector of dimension embed_dim
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2) #Output shape: [B, embed_dim, 14, 14] for a 224×224 image
        # and 16×16 patches. ps: 14 = 224/16
        return x
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False, **kwargs):
    model = TransReID(
